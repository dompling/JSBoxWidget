class Calendar {
  constructor(kernel, setting) {
    ...
    if (this.hasHoliday && $file.exists(this.setting.holidayPath)) {
      // 假期信息
      this.holiday = JSON.parse(
        $file.read(this.setting.holidayPath).string,
      ).holiday;
    }
      ...
      for (let day = 0; day <= 6; day++) {
        ...
        if (lunar && formatDate !== 0) {
          ...
          formatDate['lunar'] =
            date === dateNow
              ? this.lunar
              : this.sloarToLunar(year, formatDate.month + 1, formatDate.date);
        }
        // 节假日
        if (this.hasHoliday && formatDate !== 0) {
          // 判断是否需要展示节假日
          // month是0-11，故+1
          let holiday = this.isHoliday(
            year,
            formatDate.month + 1,
            formatDate.date,
          );
  ...
  formatDay(date, calendarInfo, hasExtra) {
    ...
    if (date.date === calendarInfo.date) {
      ...
      if (!date.holiday) {
        ...
        else props.box.background = $color(this.holidayNoRestColor);
      }
    }
    // 本月前后补位日期
  ...
  weekIndexTemplate() {
    ...
    for (let i = 0; i < 7; i++) {
      title.push(
        this.dayTemplate(this.localizedWeek(i), {
  ...
  titleBarTemplate(family, calendarInfo) {
    ...
    let left = '',
      right = '';
    if (this.titleYear) {
      let year = !this.titleFullYear
        ? String(calendarInfo.year).slice(-2) + $l10n('YEAR')
        : calendarInfo.year + $l10n('YEAR');
  ...
  calendarView(family, reviseFamily) {
    ...
    let calendar = this.formatCalendar(
      family,
      calendarInfo,
