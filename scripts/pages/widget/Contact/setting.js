...
class CurrentSetting extends Setting {
  ...
  getContactInfo = () => {
    return $cache.get(`contact_detail_${this.get('contact')}`);
  };
  ...
  getContact = (update) => {
    $contact.pick({
      multi: false,
      handler: (contact) => {
        update('contact', contact.familyName);
        $cache.set(`contact_detail_${contact.familyName}`, contact);
        $(`text_contact`).text = contact.familyName;
      },
    });
  };
