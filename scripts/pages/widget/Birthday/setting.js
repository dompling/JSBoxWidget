...
class CurrentSetting extends Setting {
  ...
  clearBackgroundImage(key = 'background') {
    $file.list(this.path).forEach((file) => {
      if (file.slice(0, file.indexOf('.')) === key) {
        console.log($file.exists(`${this.path}/${file}`));
          ...
          case 0:
            $photo.pick({
              ...
              handler: (resp) => {
                ...
                const savePath = `${this.path}/${fileName}`;
                $file.write({
                  data: image,
                  path: savePath,
                });
                update('avatar', savePath);
                $(id).image = $image(savePath);
              },
            });
            break;
          case 1:
  ...
  initSettingMethods() {
    ...
    this.setting.chooseAvatar = (update, key, name) => {
      this.chooseAvatar(update, 'avatar', `setting-image-${name}-${key}-image`);
    };
